...
describe('test sort-arrow', () => {
  test('default test', () => {
    ...
    const { getByText, getByTestId, baseElement } = render(<Demo onChange={fn}/>);

    const sortArrow = getByTestId('default-sort');
  ...
  test('mode change test', () => {
    const {baseElement, getByTestId, rerender} = render(<SortArrow mode="expand" data-testid="mode-sort"/>);
    const sortArrow = getByTestId('mode-sort');
    const iconSpan = baseElement.getElementsByClassName('i-span');
    ...
    rerender(<SortArrow mode="switch" data-testid="mode-sort" icon={['0', '1']}/>);
