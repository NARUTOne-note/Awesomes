  ...
  if (fs.existsSync(dir)) {
    ...
    files.forEach((file) => {
      ...
      if (stat && stat.isDirectory()) {
        ...
        if (ext === '.less' || ext === '.css') {
          const less = path.relative(src, sub); // 相对路径
...
function rollup() {
  ...
  console.log(chalk.blue('Generate "style/components.less"'));
  let content = '';
  fs.readdir(src, (err, files) => {
    ...
    const sourcelib = path.join(lib, 'style', 'components.less');
    const distSource = path.join(es, 'style', 'index.less');

    fs.writeFileSync(source, content);
    fs.writeFileSync(sourcelib, content);

    compile(source, path.join(es, 'style', 'components.css'));
    compile(sourcelib, path.join(lib, 'style', 'components.css'));
    compile(distSource, path.join(dist, 'index.css'));
  });
}

console.log(chalk.green('Build less files'));
fs.readdir(src, (err, files) => {
  files.forEach((file) => {
    const dir =
